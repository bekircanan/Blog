...
<?php
    if(isset($_GET['recherche'])){
        ...
        $stmt = $conn->prepare("SELECT a.id_article, a.titre, a.date_pub, SUBSTR(a.contenu, 1, 150) as contenu, u.pseudo  
                                FROM article a JOIN user u ON a.id_user = u.id_user 
                                               JOIN article_categorie ac ON ac.id_article = a.id_article
                                               JOIN categorie c ON ac.id_categorie = ac.id_categorie
                                WHERE u.pseudo LIKE :pseudo 
                                OR a.titre LIKE :titre
                                OR c.nom_categorie LIKE :categorie order by a.date_pub asc");
